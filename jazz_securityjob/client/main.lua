...
function TakeAnim()
    ...
    QBCore.Functions.Progressbar("bag_pickup", Lang:t("info.picking_bag"), math.random(3000, 5000), false, true, {
        ...
        LoadAnimation('missfbi4prepp1')
        TaskPlayAnim(ped, 'missfbi4prepp1', '_bag_walk_garbage_man', 6.0, -6.0, -1, 49, 0, 0, 0, 0)
        garbageObject = CreateObject(`prop_money_bag_01`, 0, 0, 0, true, true, true)
    ...
    CreateThread(function()
        while ControlListen do
            if IsControlJustReleased(0, 38) then
                TriggerEvent("jazz_seurityjob:client:MainMenu")
            end
            Wait(1)
        end
    end)
    ...
    for i = 1, #Config.Peds do
        ...
        if Config.UseTarget then
            exports['qb-target']:AddTargetEntity(ped, {
                options = {{type = "client", event = "jazz_seurityjob:client:MainMenu", label = Lang:t("target.talk"), icon = 'fa-solid fa-recycle', job = "garbage",}},
                distance = 2.0
            })
        else
...
RegisterNetEvent('jazz_seurityjob:client:RequestRoute', function()
    if garbageVehicle then continueworking = true TriggerServerEvent('securityjob:server:PayShift', continueworking) end
                ...
                for _,v in pairs(Config.Locations["vehicle"].coords) do
                    if not IsAnyVehicleNearPoint(vector3(v.x,v.y,v.z), 2.5) then
                        QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                            ...
                            TriggerServerEvent("jazz_seurityjob:server:payDeposit")
...
RegisterNetEvent('jazz_seurityjob:client:RequestPaycheck', function()
...
RegisterNetEvent('jazz_seurityjob:client:MainMenu', function()
    if playerJob.name == Config.Jobname then
        local MainMenu = {}
        MainMenu[#MainMenu+1] = {isMenuHeader = true,header = Lang:t("menu.header")}
        MainMenu[#MainMenu+1] = { header = Lang:t("menu.collect"),txt = Lang:t("menu.return_collect"),params = { event = 'jazz_seurityjob:client:RequestPaycheck',}}
        if not garbageVehicle or finished then
            MainMenu[#MainMenu+1] = { header = Lang:t("menu.route"), txt = Lang:t("menu.request_route"), params = { event = 'jazz_seurityjob:client:RequestRoute',}}
        end
